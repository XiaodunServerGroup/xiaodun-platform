...
class CalendarStore(ModuleStoreWriteBase):
    """
    ...
    def __init__(self, mappings, stores, reference_type=None, i18n_service=None, **kwargs):
        ...
        uses.
        """
        super(CalendarStore, self).__init__(**kwargs)
    ...
    def range_events(self,course_id,start,end):
        coll = self.modulestores["default"].collection
        events_list = []
        for event in coll.find({"course_id":course_id,"start":{"$lt":end,"$gt":start}}):
            events_list.append(event)
        return events_list


    def save_event(self,course_id,data):
        coll = self.modulestores["default"].collection
        event_id = str(time.time())
        # objectid = None
        # if coll.find_one({"id": course_id}) is None:
        coll.insert({"id":event_id, "course_id":course_id,"start":data["start"],"calendar":data})
        # else:

        return data

    def update_event(self,course_id,event_id,data):
        coll = self.modulestores["default"].collection
        # objectid = None
        # if coll.find_one({"id": course_id}) is None:
        coll.update({'id':event_id}, {'$set':{"course_id":course_id,"start":data["start"],"calendar":data}})
        # else:
